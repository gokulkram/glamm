...
export default function AdminSettingsForm({ profile }: { profile: AdminProfile }) {
  ...
  const changePassword = async (e: React.FormEvent) => {
    ...
    setSavingPw(true)
    const { error } = await createSupabaseBrowserClient().auth.updateUser({ password: pw.next })
    setSavingPw(false)
    if (error) {
    ...
    <div className="space-y-8 max-w-2xl">
      ...
      <form onSubmit={saveProfile} className="card p-6 space-y-4">
        ...
        <div>
          ...
          <input value={profile.role} disabled className={`${field} bg-surface text-text-muted capitalize`} />
        </div>
